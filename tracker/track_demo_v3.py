...
tracker_name = 'sort'  # sort, deepsort, botsort, c_bioutrack, bytetrack

testdata = "multi_drone_241016_001"


video_path = f'D:/Project/Yolov7-tracker-2/video/{testdata}.mp4'
weights = './weights/best_AGDS_v001.pt'

# Kalman, sort, deepsort, byte

if tracker_name == 'sort':
    kalman_format_name = 'sort'

elif tracker_name == 'deepsort':
    kalman_format_name = 'byte'

elif tracker_name == 'bytetrack':
    kalman_format_name = 'byte'

elif tracker_name == 'botsort':
    kalman_format_name = 'bot'

elif tracker_name == 'c_bioutrack':
    kalman_format_name = 'bot'



results_json = {key: [] for key in trackers}
fps_dict = {key: 0 for key in trackers}
...
def get_args():
    ...
    """general"""
    # parser.add_argument('--obj', type=str, required=True, default='D:\Project\Yolov7-tracker-2\video', help='video or images folder PATH')

    parser.add_argument('--obj', type=str, default=video_path, help='video or images folder PATH')

    parser.add_argument('--detector', type=str, default='yolov7', help='yolov7, yolox, etc.')
    parser.add_argument('--tracker', type=str, default = tracker_name, help='sort, deepsort, etc')
    # parser.add_argument('--reid_model', type=str, default='deepsort', help='osnet or deepsort')
    parser.add_argument('--reid_model', type=str, default='osnet_x0_25', help='osnet or deepsort')

    parser.add_argument('--kalman_format', type=str, default=kalman_format_name, help='use what kind of Kalman, sort, deepsort, byte, etc.')
    ...
    """model path"""
    parser.add_argument('--detector_model_path', type=str, default = weights, help='model path')
    parser.add_argument('--trace', type=bool, default=True, help='traced model of YOLO v7')
    # other model path
    ...
    parser.add_argument('--min_area', type=float, default=200, help='use to filter small bboxs')
    ...
    parser.add_argument('--save_videos', default=True, help='save tracking results (video)')
...
def main(args):
    ...
    for frame_idx, (ori_img, img) in process_bar:
        ...
        results_json[tracker_name].append((p1, p2))
    ...
    # 寫入 pred.txt（MOT 格式）
    # 設定縮放比例（模型輸入 vs 原圖大小）
    re_img_h, re_img_w = ori_img.shape[:2]
    org_img_h, org_img_w = 720, 1280
    scale_x = org_img_h / re_img_h
    scale_y = org_img_w / re_img_w

    # Check if the dataset folder exists, if not, create it
    dataset_folder = os.path.join(save_dir, testdata)
    if not os.path.exists(dataset_folder):
        os.makedirs(dataset_folder)

    pred_path = os.path.join(save_dir, f"{testdata}\\{tracker_name}_pred.txt")
    with open(pred_path, "w") as f:
        for frame_idx, ids, boxes, classes, scores in results:
            for i in range(len(ids)):
                x, y, w, h = map(float, boxes[i])  # 假設為 TLWH（像素座標）
                ...
                f.write(f"{frame_idx},{obj_id},{x:.2f},{y:.2f},{w:.2f},{h:.2f},{conf:.2f},{cls},1\n")
    logger.info(f"Saved MOT-format tracking result to: {pred_path}")
    ...
    logger.info(f"Total time: {total_time:.2f} seconds, FPS: {fps:.2f}")  
    ...
    results_path = f'./{save_dir}/{testdata}/tracker_results_{tracker_name}.json'

    with open(results_path, 'w') as f:
        json.dump(tracker_results, f, ensure_ascii=False, indent=4)


    print(f"Tracker results and FPS dictionary saved to {results_path}")
